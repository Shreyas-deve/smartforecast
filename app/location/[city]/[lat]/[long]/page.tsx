...
import InformatioPanel from "@/components/InformationPanel";
...
async function WeatherPage({ params: { city, long, lat } }: Props) {
  ...
  const GPTata = await res.json();
  const { content } = GPTata;
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
            ...
            <div>
              ...
              {Number(results.daily.uv_index_clear_sky_max[0].toFixed(1)) >
                5 && (
                <CalloutCard message={"The UV is High. Take Care!"} warning />
              )}
            </div>
